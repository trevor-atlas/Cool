...
function CoolLine:ADDON_LOADED(a1)
    ...
    panel:SetScript("OnShow", function(this)
        ...
        t2:SetFormattedText("Notes: %s\nAuthor: %s\nVersion: %s\n" ..
            ...
            GetAddOnMetadata("CoolLine", "Notes") or "N/A", GetAddOnMetadata("CoolLine", "Author") or "N/A",
    ...
    createfs = function(f, text, offset, just)
        ...
        if just then
            ...
            if db.vertical then
                ...
                just = db.reverse and ((just == "LEFT" and "TOP") or "BOTTOM") or
                           ((just == "LEFT" and "BOTTOM") or "TOP")
    ...
    updatelook = function()
        ...
        self.overlay = self.overlay or
                           CreateFrame("Frame", nil, self.border, BackdropTemplateMixin and "BackdropTemplate")
        ...
        SetValue = (db.vertical and (db.reverse and SetValueVR or SetValueV)) or
                       (db.reverse and SetValueHR or SetValueH)
        ...
        if not self.cb and (not smed:IsValid("font", db.font) or not smed:IsValid("border", db.border) or
            not smed:IsValid("statusbar", db.statusbar)) then
...
local iconback = {
    bgFile = "Interface\\AddOns\\CoolLine\\backdrop.tga"
}
...
do -- cache spells that have a cooldown
    local GetSpellBookItemName, GetSpellBookItemInfo, GetSpellBaseCooldown, GetSpellCharges, GetNumSpellTabs,
        GetSpellTabInfo = GetSpellBookItemName, GetSpellBookItemInfo, GetSpellBaseCooldown, GetSpellCharges,
        ...
        if frame.name == spell then
            if frame.endtime - GetTime() > 1 then
                if not failborder then
                    failborder =
                        CreateFrame("Frame", nil, CoolLine.border, BackdropTemplateMixin and "BackdropTemplate")
